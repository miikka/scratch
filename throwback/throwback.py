...
def append_links(
    ...
    *,
    ...
    if not new_lines or diary_appended == 0:
        return 0
...
def main() -> None:
    ...
    diary_count = append_links(
        today_note, vault_path, diary_dir, targets, dry_run=args.dry_run
    )

    if diary_count == 0:
        print("No historical entries found to append.")
    elif args.dry_run:
        print(f"Dry run: would append {diary_count} diary link(s).")
    else:
        print(f"Appended {diary_count} diary link(s).")
